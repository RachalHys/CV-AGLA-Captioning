...
def get_llm_load_kwargs(precision = "fp16"):
    if not torch.cuda.is_available():
        return {"torch_dtype": torch.float32}

    cap = torch.cuda.get_device_capability()
    has_bf16 = torch.cuda.is_bf16_supported()
    best_dtype = torch.bfloat16 if has_bf16 else torch.float16
    device_map = {"": 0} if torch.cuda.is_available() else None
    
    match precision:
        case "bf16" if has_bf16:
            return {
                "torch_dtype": torch.bfloat16,
                "device_map": device_map
            }
            
        case "int8" if cap >= (7, 0):
            return {
                "quantization_config": BitsAndBytesConfig(load_in_8bit=True, llm_int8_threshold=6),
                "device_map": device_map,
                "torch_dtype": torch.float16,
            }
            
        case "int4" if cap >= (7, 5):
            return {
    ...
    #Fallback
    logging.warning(f"Precision '{precision}' not supported or invalid. Falling back to fp16.")
    return {
        "torch_dtype": torch.float16,
        "device_map": device_map
    }
...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def maybe_autocast(self, dtype=None):
        # Prefer BF16 when supported; otherwise safely fall back to FP16.
        if dtype is None:
            dtype = self.runtime_dtype
        return super().maybe_autocast(dtype=dtype)
    ...
    def __init__(
        ...
        load_kwargs = get_llm_load_kwargs(precision)
        self.llm_model = LlamaForCausalLM.from_pretrained(llm_model, **load_kwargs)
        ...
        self.visual_encoder.to(self.runtime_dtype)
        self.ln_vision.to(self.runtime_dtype)
        self.Qformer.to(self.runtime_dtype)
        self.llm_proj.to(self.runtime_dtype)
        
        self.llm_model.get_input_embeddings().to(self.runtime_dtype)
    ...
    def from_config(cls, cfg):
        ...
        if vit_precision == "bf16" and not (torch.cuda.is_available() and torch.cuda.is_bf16_supported()):
            logging.warning("Requested vit_precision=bf16 but BF16 is unavailable. Falling back to fp16.")
            vit_precision = "fp16"
