...
def sample(
    ...
    **model_kwargs,
    ...
    cd_output_hidden_states = (
        output_hidden_states if output_hidden_states is not None else self.generation_config.output_hidden_states
    )

    # initialise model_kwargs_cd before the loop. Each branch
    ...
    if use_cd:
        ...
        model_kwargs_cd = {k: v for k, v in model_kwargs.items()}
    ...
    while True:
        ...
        if use_cd:
            # model_kwargs_cd carries its own past_key_values, so KV-caching works correctly for both
            ...
            outputs_cd = self(
                **model_inputs_cd,
                return_dict=True,
                output_attentions=cd_output_attentions,
                output_hidden_states=cd_output_hidden_states,
            )
        ...
        if return_dict_in_generate:
            if output_scores:
                # next_token_scores only defined in else-branch (use_cd=False).
                # Use cd_logits for the CD branch to avoid NameError.
                scores += (cd_logits if use_cd else next_token_scores,)
