...
def get_args():
    ...
    parser.add_argument("--top_k", type=int, default=10, help="Number of worst images to save")
...
def main(args):
    ...
    for i in tqdm(range(len(inference_data))):
        ...
        if ground_truth[id-1]['type'] == 'generative':
            ...
            hallucination_count = sum(safe_flag_list)
    ...
    error_samples = sorted(error_samples, key=lambda x: x['hallucination_count'], reverse=True)
